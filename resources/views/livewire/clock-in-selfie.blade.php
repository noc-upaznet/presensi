...
<div>
    ...
    <div class="content-wrapper p-4">
        ...
        @if ($errorMessage)
            <div class="alert alert-danger alert-dismissible fade show mt-2" role="alert" x-data x-init="setTimeout(() => $el.remove(), 4000)">
                {{ $errorMessage }}
                            ...
                            <div class="mb-3">
                                ...
                                @foreach ($lokasisTerdekat as $lokasi)
                                    <li class="badge bg-info text-dark me-1">
                                        {{ $lokasi->nama_lokasi }}
                                        <small class="text-muted">({{ number_format($lokasi->jarak * 1000, 0) }} m)</small>
                                    </li>
                                @endforeach
                            </div>
                            <button class="btn btn-danger btn-small" 
                                    wire:click="reloadLocation"
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        Livewire.dispatch('lokasiAwal', { latitude: lat, longitude: lng });
                    },
                    (error) => {
                        console.error('Gagal mendapatkan lokasi:', error);
                        Livewire.dispatch('lokasiError', { message: 'Tidak dapat mengakses lokasi. Aktifkan GPS Anda.' });
                    }
                );
            } else {
                Livewire.dispatch('lokasiError', { message: 'Browser Anda tidak mendukung geolokasi.' });
            }
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        (error) => {
                            ...
                            Livewire.dispatch('lokasiError', { message: 'Tidak dapat memperbarui lokasi. Aktifkan GPS Anda.' });
                        }
                    );
                } else {
                    Livewire.dispatch('lokasiError', { message: 'Browser Anda tidak mendukung geolokasi.' });
