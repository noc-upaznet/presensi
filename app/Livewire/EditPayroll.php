...
class EditPayroll extends Component
{
    ...
    public function hitungRekap($userId)
    {
        ...
        $this->izin        = ($izin ?? 0)
            + (0.5 * ($izinSetengahHari ?? 0))
            + (0.5 * ($izinSetengahHariPagi ?? 0))
            + (0.5 * ($izinSetengahHariSiang ?? 0))
            + (0.5 * ($KonterizinSetengahHariPagi ?? 0));
        $this->cuti        = $cuti ?? 0;
    ...
    public function loadData($id)
    {
        ...
        $this->kebudayaan = $this->karyawan->kebudayaan;
        ...
        $this->transport = $this->karyawan->transport;
        ...
        $this->voucher = $this->karyawan->voucher;
    ...
    public function isSalesPosition()
    {
        ...
        if ($this->karyawan) {
            return $this->karyawan->divisi === 'Sales Marketing'
                && $this->karyawan->level !== 'SPV'; // exclude SPV
        }
        return false;
    }
            ...
            for ($i = 1; $i <= 31; $i++) {
                ...
                if (isset($jadwal->$col)) {
                    if ($jadwal->$col == 2) {
                        ...
                        $izinSetengahHariSiang++;
                    } elseif ($jadwal->$col == 29) { // kode 29 = izin setengah hari siang
                        $KonterizinSetengahHariPagi++;
                    }
                }
            }
    ...
    public function hitungTotalGaji()
    {
        ...
        $voucher            = $this->numericValue($this->voucher ?? 0);
